...
class GenerationModel(Model, Scorer):
    def encode_and_generate(
        ...
        **generation_args
        ...
        imgs = self.processor(text=vm_prompt, images=[image_good, *images_bad],
                              padding=True,
    ...
    def run(self, img, instruction, device: str = 'cuda:0',
        ...
        with torch.autocast("cuda"):
            ...
            if do_format:
                vm_prompt = self.format_vm_prompt(instruction_vm, False)
                if prefix_vm is not None:
                    vm_prompt += prefix_vm
            else:
                vm_prompt = instruction_vm
        ...
        if return_prompt:
            return out, [lm_prompt, vm_prompt]
        return out

    def format_vm_prompt(self, instruction, lower_text: bool = False):
        instruction = instruction.strip()
        if not (instruction.endswith('.') or instruction.endswith('?')):
            instruction = f'{instruction}.'
        qs = instruction
        text_input = vm_template.format(qs).strip()

        return text_input.lower() if lower_text else text_input
